...
```{r}
train_nat = add_column(train_nat, train_pob$poblacion_base) 
colnames(train_nat)[3] = "poblacion" 
colnames(train_nat)[1] = "natalidad"

test_nat = add_column(test_nat, test_pob$poblacion_base)
...
```{r}
fit_lm <- train_nat |>
  model(tslm = TSLM(log(poblacion) ~ trend() + natalidad))
