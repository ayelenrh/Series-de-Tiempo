...
poblacion_gdp = poblacion_tidy %>% mutate(poblacion_base = cumsum(Poblacion))
poblacion_gdp
```

```{r}
train_nat <- natalidad_gdp %>% select(natalidad_base) %>% filter_index("1980" ~ "2018")
test_nat <- natalidad_gdp %>% select(natalidad_base) %>% filter_index("2019" ~ "2022")
train_nat
test_nat

train_pob <- poblacion_gdp %>% select(poblacion_base) %>% filter_index("1980" ~ "2018")
test_pob <- poblacion_gdp %>% select(poblacion_base) %>% filter_index("2019" ~ "2022")
train_pob
test_pob
```

```{r}
train_nat = add_column(train_nat, poblacion_tidy$poblacion_base) 
colnames(train_nat)[3] = "poblacion" 
colnames(train_nat)[1] = "natalidad"

test_nat = add_column(test_nat, poblacion_tidy$poblacion_base)
